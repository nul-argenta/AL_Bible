...
async function verifyCSV(filePath, expectedColumns) {
    ...
    for await (const line of rl) {
        ...
        for (let i = 0; i < line.length; i++) {
            if (line[i] === '"') {
                // If it's a double quote "", it's an escaped quote within a quoted field.
                // Our simple logic: toggle inQuotes on every quote. This correctly handles "" too.
                inQuotes = !inQuotes;
    ...
    console.log(`[OK] ${filePath.padEnd(30)} | Lines: ${String(lineCount-1).padEnd(6)} | Errors: ${errors}`);
    if (errors > 0 || inQuotes) {
